...
@app.route('/api/genetico', methods=['POST'])
def genetico_api():
    data = request.form

    max_iter = int(data.get('max_iter', 10))
    max_poblacion = int(data.get('max_poblacion', 50))
    num_vars = int(data.get('num_vars', 10))

    random.seed()
    mejor_gen_resultados = algoritmo_genetico(max_iter, max_poblacion, num_vars)

    return render_template('resultado.html', mejor_ruta=mejor_gen_resultados[0], res=mejor_gen_resultados[1])
